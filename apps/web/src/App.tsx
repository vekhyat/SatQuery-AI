...
import { Tool1SingleView, isTool1SingleResult } from './components/Tool1SingleView';
...
export default function App() {
  ...
  const isTool1Specialist = !isDemo && isTool1SingleResult(result);
  ...
  return <div className="app-shell">
    ...
    <div className="app-body">
      ...
      <main id="workspace" className="workspace">
        ...
        {helpOpen && <section className="help-panel"><div><p className="help-title">From question to evidence.</p><p>Attach one or two GeoTIFFs and ask a question. The router selects the analysis from your files. Open the receipt to inspect each step.</p><p><strong>Illustrative data</strong> is a prepared example. Your own scenes call the local API. Single-image reports land-cover classes from spectral indices (not calibrated accuracy). Change uses the local model worker. Optical + SAR produces threshold-based candidate maps from named multispectral bands and calibrated radar.</p></div><button className="icon-button" type="button" aria-label="Close help" onClick={() => setHelpOpen(false)}><X size={18} /></button></section>}
          ...
          {!emptyLive && <div className={'evidence-stack' + (showCallout ? ' has-callout' : '')}>
            ...
            {tool3Maps && result ? <Tool3Maps key={tool3Maps.fused} result={result} urls={tool3Maps} busy={busy} />
              : isTool2Specialist && result ? <Tool2ChangeView result={result} scenes={current.scenes} busy={busy} />
                : isTool1Specialist && result ? <Tool1SingleView key={result.overlay?.file ?? 'tool1'} result={result} scene={current.scenes[0]} busy={busy} />
