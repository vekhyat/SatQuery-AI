...
class DecoderTransformer(nn.Module):
    ...
    def forward(self, x1, x2, encoded_captions, caption_lengths):
        ...
        mask = torch.triu(
            torch.full((word_length, word_length), float('-inf'), device=x.device),
            diagonal=1,
        )
    ...
    def sample(self, x1, x2, k=1):
        ...
        device = x.device
        tgt = torch.zeros(batch, self.max_lengths, dtype=torch.int64, device=device) #(batch_size, self.max_lengths)

        mask = torch.triu(
            torch.full((self.max_lengths, self.max_lengths), float('-inf'), device=device),
            diagonal=1,
        )
        tgt[:, 0] = self.word_vocab['<START>'] #(batch_size, 1)
        seqs = torch.full((batch, 1), self.word_vocab['<START>'], dtype=torch.int64, device=device) #(batch_size, 1)
    ...
    def sample_beam(self, x1, x2, k=1):
        ...
        device = x.device
        tgt = torch.zeros(k*batch, self.max_lengths, dtype=torch.int64, device=device) #(batch_size*k, self.max_lengths)

        mask = (torch.triu(torch.ones(self.max_lengths, self.max_lengths)) == 1).transpose(0, 1)
        mask = mask.float().masked_fill(mask == 0, float('-inf')).masked_fill(mask == 1, float(0.0))
        mask = mask.to(device)
        tgt[:, 0] = self.word_vocab['<START>'] #(batch_size*k, 1)
        seqs = torch.full((batch*k, 1), self.word_vocab['<START>'], dtype=torch.int64, device=device)
